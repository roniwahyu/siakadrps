		...
		<div class="col-md-6 col-12">
			<div class="my-5 card-body">
				@if(session()->has('message'))
					...
					<i class="text-success fa fa-check-circle"></i> {{ session()->get('message') }}
					</div>
				@endif
				
				<div class="row">
					<div class="col-md-auto col-12 text-center">
						<i class="mi-lg text-success fa fa-check-circle"></i>
					</div>
					<div class="col">
						<h3 class="text-primary mb-2 font-weight-bold">{{ __('otpVerification') }}</h3>
						<p class="text-muted">
							{{ __('otpSentSMS') }}
						</p>
					</div>
				...
				<form method="POST" action="{{ route('auth.validateotp') }}">
					...
					<div class="form-group mb-3">
						<input name="otp_code" type="text" class="form-control otp-input" required autofocus placeholder="{{ __('enterOtp') }}">
					</div>
