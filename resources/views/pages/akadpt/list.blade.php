...
<?php
    ...
    $can_add = $user->canAccess("akadpt/add");
    $can_edit = $user->canAccess("akadpt/edit");
    $can_view = $user->canAccess("akadpt/view");
    $can_delete = $user->canAccess("akadpt/delete");
    $field_name = request()->segment(3);
    $field_value = request()->segment(4);
    $total_records = $records->total();
    $limit = $records->perPage();
    $record_count = count($records);
    $pageTitle = __('akadPt'); //set dynamic page title
?>
@extends($layout)
    ...
    <div  class="bg-light p-3 mb-3" >
        <div class="container-fluid">
            <div class="row justify-content-between align-items-center gap-3">
                <div class="col  " >
                    <div class="">
                        <div class="h5 font-weight-bold text-primary">{{ __('akadPt') }}</div>
                    </div>
                </div>
                <div class="col-auto  " >
                    <?php if($can_add){ ?>
                            ...
                            <table class="table table-hover table-striped table-sm text-left">
                                <thead class="table-header ">
                                    <tr>
                                        ...
                                        <?php } ?>
                                        <th class="td-kode_pt" > {{ __('kodePt') }}</th>
                                        <th class="td-nm_pt" > {{ __('nmPt') }}</th>
                                        <th class="td-tgl_pt" > {{ __('tglPt') }}</th>
                                        <th class="td-sk_pt" > {{ __('skPt') }}</th>
                                        <th class="td-tgl_sk_pt" > {{ __('tglSkPt') }}</th>
                                        <th class="td-almt_pt" > {{ __('almtPt') }}</th>
                                        <th class="td-kota_pt" > {{ __('kotaPt') }}</th>
                                        <th class="td-kodepos_pt" > {{ __('kodeposPt') }}</th>
                                        <th class="td-telp_pt" > {{ __('telpPt') }}</th>
                                        <th class="td-fax_pt" > {{ __('faxPt') }}</th>
                                        <th class="td-email_pt" > {{ __('emailPt') }}</th>
                                        <th class="td-web_pt" > {{ __('webPt') }}</th>
                                        <th class="td-logo_pt" > {{ __('logoPt') }}</th>
                                ...
                                <tbody class="page-data">
                                    ...
                                    <tr>
                                        ...
                                        <td class="td-btn">
                                            ...
                                            <?php } ?>
                                            <?php if($can_delete){ ?>
                                            <a class="dropdown-item record-delete-btn" data-prompt-msg="{{ __('promptDeleteRecord') }}" data-display-style="modal" href="<?php print_link("akadpt/delete/$rec_id"); ?>" >
            ...
            <div class=" mt-3">
                <div class="row align-items-center justify-content-between">    
                    <div class="col-md-auto d-flex">    
                        <?php if($can_delete){ ?>
                        <button data-prompt-msg="{{ __('promptDeleteRecords') }}" data-display-style="modal" data-url="<?php print_link("akadpt/delete/{sel_ids}"); ?>" class="btn btn-sm btn-danger btn-delete-selected d-none">
