...
<?php
    ...
    $can_add = $user->canAccess("rpsrps/add");
    $can_edit = $user->canAccess("rpsrps/edit");
    $can_view = $user->canAccess("rpsrps/view");
    $can_delete = $user->canAccess("rpsrps/delete");
    $field_name = request()->segment(3);
    $field_value = request()->segment(4);
    $total_records = $records->total();
    $limit = $records->perPage();
    $record_count = count($records);
    $pageTitle = __('rpsRps'); //set dynamic page title
?>
@extends($layout)
    ...
    <div  class="bg-light p-3 mb-3" >
        <div class="container-fluid">
            <div class="row justify-content-between align-items-center gap-3">
                <div class="col  " >
                    <div class="">
                        <div class="h5 font-weight-bold text-primary">{{ __('rpsRps') }}</div>
                    </div>
                </div>
                <div class="col-auto  " >
                    <?php if($can_add){ ?>
                    <a  class="btn btn-primary btn-block" href="<?php print_link("rpsrps/add", true) ?>" >
                ...
                <div  class=" page-content" >
                    <div id="rpsrps-list-records">
                        <div id="page-main-content" class="table-responsive">
                            ...
                            <?php Html::page_bread_crumb("/rpsrps/", $field_name, $field_value); ?>
                            ...
                            <table class="table table-hover table-striped table-sm text-left">
                                <thead class="table-header ">
                                    <tr>
                                        ...
                                        <th class="td-id_otoritas1" > {{ __('idOtoritas1') }}</th>
                                        <th class="td-id_otoritas2" > {{ __('idOtoritas2') }}</th>
                                        <th class="td-deskripsi_rps" > {{ __('deskripsiRps') }}</th>
                            ...
                            <td class="td-btn">
                                <div class="dropdown" >
                                    ...
                                    <ul class="dropdown-menu">
                                        <?php if($can_view){ ?>
                                        <a class="dropdown-item "   href="<?php print_link("rpsrps/view/$rec_id"); ?>" >
                                        <i class="fa fa-eye"></i> {{ __('view') }}
                                    </a>
                                    <?php } ?>
                                    <?php if($can_edit){ ?>
                                    <a class="dropdown-item "   href="<?php print_link("rpsrps/edit/$rec_id"); ?>" >
                                    <i class="fa fa-edit"></i> {{ __('edit') }}
                                </a>
                                <?php } ?>
                                <?php if($can_delete){ ?>
                                <a class="dropdown-item record-delete-btn" data-prompt-msg="{{ __('promptDeleteRecord') }}" data-display-style="modal" href="<?php print_link("rpsrps/delete/$rec_id"); ?>" >
...
<div class=" mt-3">
    <div class="row align-items-center justify-content-between">    
        <div class="col-md-auto d-flex">    
            <?php if($can_delete){ ?>
            <button data-prompt-msg="{{ __('promptDeleteRecords') }}" data-display-style="modal" data-url="<?php print_link("rpsrps/delete/{sel_ids}"); ?>" class="btn btn-sm btn-danger btn-delete-selected d-none">
